...
- **Portals** (v0.16.1 / v0.16.2) save a web-UI target — base URL + login cookies + selectors + "response done" predicate — under a name. `clawtool portal add/list/use/which/unset/remove/ask` plus the matching `Portal*` MCP tools manage them. v0.16.2 ships the CDP driver: `portal ask` spawns `obscura serve`, drives the saved flow, and returns the rendered response. Per-portal `<name>__ask` aliases register at `clawtool serve` boot so an MCP-aware model can call them directly. See [docs/portals.md](docs/portals.md) and [ADR-018](wiki/decisions/018-portal-feature.md).
