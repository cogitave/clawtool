...
- **MCP authoring** (v0.16.4 surface, v0.17 generator) `clawtool mcp new <project>` will scaffold a fresh MCP server (Go via `mcp-go`, Python via `fastmcp`, TypeScript via `@modelcontextprotocol/sdk`) — wizard picks language / transport / packaging / tools, optionally writes a `.claude-plugin/` manifest, and `clawtool mcp install` registers it as `[sources.X]`. Sister of `clawtool skill new` (Agent Skills). Surface stub + ToolSearch registration land today; generator follows. See [docs/mcp-authoring.md](docs/mcp-authoring.md) and [ADR-019](wiki/decisions/019-mcp-authoring-scaffolder.md).
