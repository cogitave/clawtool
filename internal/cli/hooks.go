...
func (a *App) HooksList(format listfmt.Format) error {
	...
	// (sister of skill / source / sandbox / portal list): table
	// mode emits the human hint, JSON + TSV route through Render
	// so pipelines see `[]\n` / a header line.
	return listfmt.RenderOrHint(a.Stdout, format, cols, "(no hooks configured — see https://github.com/cogitave/clawtool#hooks for examples)")
}
