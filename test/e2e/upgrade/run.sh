...
echo "$OLDV" | grep -q 'v0.0.0-old' || fail "expected v0.0.0-old, got: $OLDV"
...
step "probe /v1/health → expect v0.0.0-old"
H1=$(probe_health "$PORT" "$TOKEN") || fail "old health unreachable on :$PORT"
echo "old health: $H1"
echo "$H1" | grep -q 'v0.0.0-old' || fail "old health did not advertise v0.0.0-old"
...
echo "$NEWV" | grep -q 'v0.0.0-new' || fail "binary did not swap"
...
step "probe /v1/health → expect v0.0.0-new"
H2=$(probe_health "$PORT2" "$TOKEN2") || fail "new health unreachable on :$PORT2"
echo "new health: $H2"
echo "$H2" | grep -q 'v0.0.0-new' || fail "post-restart health did not advertise v0.0.0-new"
