...
- ✅ **Test totals: 65 Go unit tests + 29 e2e = 94 green.** Per package: catalog 11, cli 21, config 11, secrets 7, sources 7, tools/core 17 (Bash 5 + Grep 5 + Read 7).
...
- **v0.5 increments**: (1) **`source list --runtime`** flag + watcher process to surface live `Status / Reason / ToolCount` from a running clawtool serve (currently `source list` only reports config-level auth status, not runtime health); (2) **auto-restart on crash** in Manager (currently a crashed source stays Down until next clawtool start); (3) tag + group resolution in config (full ADR-004 §4 precedence); (4) `ToolSearch` BM25 baseline via `bleve`; (5) hot-reload config watcher; (6) `Edit` + `Write` core tools using OpenAI `apply_patch` format; (7) long-form `clawtool source add custom -- <command>` for sources outside the catalog; (8) `source rename` for the "first instance was bare-named, now I want to add another" workflow.
