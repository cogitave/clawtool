...
- ✅ **Test totals: 58 Go unit tests + 23 e2e = 81 green.** Per package: catalog 11, cli 21, config 11, secrets 7, tools/core 17 (Bash 5 + Grep 5 + Read 7).
...
- **v0.4 turn 2 next**: (1) **MCP client/server proxy** — clawtool spawns each configured source as a child MCP server, aggregates their tools under the wire-form naming `<instance>__<tool>`, routes tools/call to the right child, manages lifecycle (start, restart on crash, kill on shutdown). This is the piece that turns "configured" into "running" — currently `source add` only writes config, no tools are actually exposed yet from sources. After turn 2, `source list` will show real health; tools/list will include `github__create_issue` etc.
- **v0.5+ increments**: (1) tag + group resolution in config (full ADR-004 §4 precedence); (2) `ToolSearch` BM25 baseline via `bleve`; (3) hot-reload config watcher; (4) `Edit` + `Write` core tools using OpenAI `apply_patch` format; (5) long-form `clawtool source add custom -- <command>` for sources outside the catalog; (6) `source rename` for the "first instance was bare-named, now I want to add another" workflow.
